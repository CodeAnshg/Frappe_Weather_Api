...
// Load last searched city from local storage when the page loads
window.addEventListener('load', () => {
    const lastSearchedCity = localStorage.getItem('lastSearchedCity');
    if (lastSearchedCity) {
        cityInput.value = lastSearchedCity;
        search_name_Api(); // Perform a search with the last searched city
    }
});
...
const search_name_Api = () => {
    ...
    if (City_name) {
        localStorage.setItem('lastSearchedCity', City_name); // Save the city to local storage
        ...
        fetch(API_URL)
            ...
            .then((data) => {
                if (data.length > 0) {
                    ...
                    alert("City not found. Please try again."); // Alert message when city is not found
                }
            })
            .catch((error) => {
        ...
        alert("Please enter a city name."); // Alert message if input is empty
    }
}
